...
import { 
  ...
  ChevronDown, ArrowUpAZ, ArrowDownAZ, Columns
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleSendTech = async (appt: Appointment) => {
    const message = `*ORDEM DE SERVIÇO - HIDRO CLEAN*\n\n` +
      `*Cliente:* ${appt.customer_name}\n` +
      `*Serviço:* ${appt.service_type}\n` +
      `*Morada:* ${appt.description}\n` +
      `*Agenda:* ${new Date(appt.scheduled_at).toLocaleDateString('pt-PT')} das ${appt.start_time} às ${appt.end_time}\n\n` +
      `*Notas:* ${appt.notes || 'N/A'}`;
  ...
  const AppointmentCard = ({ appt }: { appt: Appointment; key?: string }) => {
    const now = new Date();
    const todayStr = new Date(now.getTime() - (now.getTimezoneOffset() * 60000)).toISOString().split('T')[0];
    ...
    return (
      ...
      >
        ...
        <div className="flex-1">
          <span className="text-blue-600 text-[9px] font-black uppercase tracking-widest block mb-1">{appt.service_type}</span>
        ...
        <div className="flex items-center gap-3">
           <button onClick={(e) => { e.stopPropagation(); handleWhatsApp(appt.phone); }} className="p-3 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white transition-all"><MessageCircle size={20}/></button>
           <button onClick={(e) => { e.stopPropagation(); handleSendTech(appt); }} className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-blue-600 hover:text-white transition-all"><Send size={20}/></button>
           <button onClick={async (e) => { e.stopPropagation(); if(confirm('Excluir lead?')) { await supabase.from('appointments').delete().eq('id', appt.id); loadCRM(); } }} className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-red-500 hover:text-white transition-all"><Trash2 size={20}/></button>
        </div>
        <div className="text-right min-w-[100px] border-l pl-6 border-slate-100">
           ...
           <div className="text-[9px] font-black text-slate-400 mt-1 uppercase">{appt.tech_sent_at ? 'Enviado' : 'Não Enviado'}</div>
        </div>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="px-8 py-4 bg-white border-b border-slate-200 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
             <button onClick={() => setIsAIModalOpen(true)} className="p-2.5 bg-slate-900 text-white rounded-xl shadow-lg"><Sparkles size={18} className="text-blue-400" /></button>
             <button onClick={() => setIsApptModalOpen(true)} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-black text-xs shadow-xl uppercase tracking-widest flex items-center gap-2"><Plus size={16}/> Novo Lead</button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-8 no-scrollbar bg-[#F3F4F7]">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="bg-white p-1 rounded-xl flex border border-slate-200 shadow-sm items-center px-2">
                    <button 
                      ...
                      className={`px-4 py-2 rounded-lg text-[9px] font-black transition-all ${timeFilter === 'today' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}
                    >
                      HOJE
                    </button>
                    <button 
                      onClick={() => setTimeFilter('semana')} 
                      className={`px-4 py-2 rounded-lg text-[9px] font-black transition-all ${timeFilter === 'semana' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}
                    >
                      SEMANA
                    </button>
                    <button 
                      onClick={() => setTimeFilter('mês')} 
                      className={`px-4 py-2 rounded-lg text-[9px] font-black transition-all ${timeFilter === 'mês' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}
                    >
                      MÊS
                    </button>
                    <button 
                      onClick={() => setTimeFilter('todos')} 
                      className={`px-4 py-2 rounded-lg text-[9px] font-black transition-all ${timeFilter === 'todos' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}
                    >
                      TODOS
                    </button>
                    <div className="h-4 w-[1px] bg-slate-200 mx-2"></div>
                    <select 
                      className="bg-transparent text-[9px] font-black uppercase outline-none cursor-pointer text-slate-600 pr-2" 
                      value={selectedMonth} 
                      onChange={e => {
              ...
              <div className="space-y-12 pb-10">
                ...
                {filteredAppointments.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-24 bg-white/50 rounded-[3rem] border-2 border-dashed border-slate-200">
                    <div className="bg-slate-200 p-8 rounded-full mb-6 text-slate-400"><Filter size={48}/></div>
                    <p className="font-black text-xs text-slate-400 uppercase tracking-[0.3em]">Nenhum pedido para {timeFilter === 'today' ? 'HOJE' : timeFilter.toUpperCase()}</p>
                    {timeFilter !== 'todos' && (
                      <button onClick={() => setTimeFilter('todos')} className="mt-4 text-[10px] font-black text-blue-600 uppercase underline hover:text-blue-800">Ver todo o histórico</button>
                    )}
                  </div>
                )}
              </div>
          ...
          {view === 'calendar' && <FullCalendarView appointments={appointments} onEditAppt={setSelectedAppt} onNewAppt={() => setIsApptModalOpen(true)} onSendTech={handleSendTech} />}
          {view === 'library' && (
          ...
          <div className="bg-white rounded-[3rem] w-full max-w-6xl h-[95vh] flex flex-col shadow-2xl border border-white overflow-hidden">
              ...
              <div className="flex items-center space-x-6 flex-1">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    ...
                    <div className="bg-blue-50 border border-blue-100 px-4 py-2 rounded-xl text-xs font-black text-blue-700 flex items-center gap-2"><FileText size={16}/> <input className="bg-transparent outline-none w-28 placeholder:text-blue-300" value={selectedAppt.nif} onChange={e => setSelectedAppt({...selectedAppt, nif: e.target.value})} placeholder="CONTRIBUINTE" /></div>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4">
                 ...
                 <button onClick={async () => { if(confirm('Eliminar lead?')) { await supabase.from('appointments').delete().eq('id', selectedAppt.id); loadCRM(); setSelectedAppt(null); } }} className="px-8 py-4 bg-white border border-slate-200 text-slate-400 rounded-2xl font-black text-xs uppercase flex items-center gap-3 hover:bg-red-500 hover:text-white transition-all"><Trash2 size={18}/> Eliminar</button>
            ...
            <div className="flex-1 p-10 grid grid-cols-12 gap-10 overflow-y-auto no-scrollbar bg-white">
              ...
              <div className="col-span-8 space-y-10">
                 <div className="grid grid-cols-2 gap-8 p-8 bg-slate-50 rounded-[3rem] border border-slate-100">
                    <div>
                    ...
                    <div>
                       ...
                       <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col space-y-5 shadow-sm">
                          <div className="flex items-center gap-4 text-xs font-black text-slate-700"><CalendarIcon size={18} className="text-blue-600"/><input type="date" className="bg-transparent outline-none w-full cursor-pointer" value={selectedAppt.scheduled_at} onChange={e => setSelectedAppt({...selectedAppt, scheduled_at: e.target.value})} /></div>
                          <div className="flex items-center justify-between border-t border-slate-100 pt-4">
                             <div className="bg-blue-50 text-blue-700 px-4 py-2 rounded-xl text-xs font-black border border-blue-100 flex items-center gap-2"><Clock size={14}/><input type="time" className="bg-transparent outline-none" value={selectedAppt.start_time} onChange={e => setSelectedAppt({...selectedAppt, start_time: e.target.value})} /></div>
                             <span className="text-[9px] font-black text-slate-300 uppercase">Até</span>
                             <div className="bg-blue-50 text-blue-700 px-4 py-2 rounded-xl text-xs font-black border border-blue-100 flex items-center gap-2"><Clock size={14}/><input type="time" className="bg-transparent outline-none" value={selectedAppt.end_time} onChange={e => setSelectedAppt({...selectedAppt, end_time: e.target.value})} /></div>
                          </div>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="bg-slate-50/50 p-8 rounded-[3rem] border border-slate-100">
                    <div className="flex items-center justify-between mb-8 px-4">
                       <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center"><Upload className="mr-3 text-blue-600" size={18}/> Anexos</h3>
                       <button className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 transition-all shadow-lg"><Plus size={16}/> Adicionar</button>
                    </div>
            ...
            <footer className="px-10 py-6 border-t bg-slate-50/30 flex items-center justify-between">
              <div className="flex items-center space-x-3 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]"><ShieldCheck size={18} className="text-emerald-500" /> Smart CRM v5.0 Platinum</div>
              <div className="flex items-center space-x-4">
                 <button onClick={() => { handleSendTech(selectedAppt); setIsTechModalOpen(true); }} className="px-10 py-5 bg-[#00A884] hover:bg-[#008F6F] text-white rounded-[1.5rem] font-black text-xs shadow-xl shadow-emerald-50 flex items-center gap-4 transition-all active:scale-95"><Send size={20}/> Enviar p/ Técnico</button>
        ...
        <div className="fixed inset-0 z-[300] bg-black/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="p-8 bg-emerald-50 border-b border-emerald-100 flex items-center justify-between">
              <h3 className="text-xl font-black text-emerald-800">Ordem de Serviço</h3>
              <button onClick={() => setIsTechModalOpen(false)} className="text-emerald-400 hover:text-emerald-600 transition-colors"><X size={24}/></button>
            </div>
            <div className="p-8 space-y-6">
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 font-mono text-xs whitespace-pre-wrap text-slate-700 leading-relaxed max-h-96 overflow-y-auto">
                {techMessage}
              </div>
              <button 
                onClick={() => {
                ...
                className={`w-full py-4 rounded-2xl font-black text-sm flex items-center justify-center gap-3 transition-all ${copiedTech ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-white hover:bg-black'}`}
              >
                {copiedTech ? <Check size={20}/> : <Copy size={20}/>}
                <span>{copiedTech ? 'Copiado para o WhatsApp!' : 'Copiar Texto da OS'}</span>
              </button>
            </div>
          </div>
        </div>
